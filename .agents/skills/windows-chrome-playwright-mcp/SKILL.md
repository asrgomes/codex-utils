...
MCP server and let `scripts/playwright-mcp-wrapper.mjs` run the readiness checks once. Do not ask
again before using Chrome for those browser-interaction instructions. Do not start or reuse Chrome
until an actual browser interaction is needed.
...
4. The wrapper fails fast if WSL2 mirrored networking, PowerShell, Windows Chrome, Node.js 24+,
   npm, npx, or port ownership checks fail.
5. The wrapper starts the dedicated Windows Chrome debug profile only if it is not already healthy.
6. The wrapper waits for `http://localhost:${WINDOWS_CHROME_CDP_PORT:-9223}/json/version`.
7. The wrapper `exec`s Playwright MCP in the foreground:
